...
void Board::getBoundNumLibertiesAfterPlay(Loc loc, Player pla, int& lowerBound, int& upperBound) const
{
  ...
  if(    colors[loc + adj_offsets[0]] == opp 
      && colors[loc + adj_offsets[1]] == opp 
      && colors[loc + adj_offsets[4]]==pla) {
    caps[0] = true;
    caps[1] = true;
  }
  if(    colors[loc + adj_offsets[0]] == opp 
      && colors[loc + adj_offsets[2]] == opp 
      && colors[loc + adj_offsets[5]]==pla) {
    caps[0] = true;
    caps[2] = true;
  }
  if(    colors[loc + adj_offsets[3]] == opp 
      && colors[loc + adj_offsets[1]] == opp 
      && colors[loc + adj_offsets[6]]==pla) {
    caps[3] = true;
    caps[1] = true;
  }
  if(    colors[loc + adj_offsets[3]] == opp 
      && colors[loc + adj_offsets[2]] == opp
      && colors[loc + adj_offsets[7]]==pla) {
    caps[3] = true;
    caps[2] = true;
  }
  ...
  lowerBound = numImmediateLibs + numCaps;
  upperBound = numImmediateLibs + numCaps;
...
int Board::getNumLibertiesAfterPlay(Loc loc, Player pla, int max) const
{
    int lowerBound = 0;
    int upperBound = 0;
    getBoundNumLibertiesAfterPlay(loc, pla, lowerBound, upperBound);
...
bool Board::isWhiteWin()const 
{
  std::vector<Loc> chain_head_start;
  std::vector<Loc> chain_head_goal;

  for(int y = 0; y < y_size; y++) {
    Loc loc_start = 1 + (y + 1) * (x_size + 1);
    Loc loc_goal = x_size + (y + 1) * (x_size + 1);
    if(colors[loc_start] == C_WHITE) {
      chain_head_start.push_back(chain_head[loc_start]);
    }
    if(colors[loc_goal] == C_WHITE) {
      chain_head_goal.push_back(chain_head[loc_goal]);
    }
  }

  for(int i = 0; i < chain_head_start.size(); ++i) {
    for(int j = 0; j < chain_head_goal.size(); ++j) {
      if(chain_head_start[i] == chain_head_goal[j]) {
        return true;
      }
    }
  }
...
bool Board::isBlackWin()const 
{
  std::vector<Loc> chain_head_start;
  std::vector<Loc> chain_head_goal;

  for(int x = 0; x < x_size; x++) {
    Loc loc_start = (x + 1) + (1) * (x_size + 1);
    Loc loc_goal = (x + 1) + y_size * (x_size + 1);
    if(colors[loc_start] == C_BLACK) {
      chain_head_start.push_back(chain_head[loc_start]);
    }
    if(colors[loc_goal] == C_BLACK) {
      chain_head_goal.push_back(chain_head[loc_goal]);
    }
  }
  for(int i = 0; i < chain_head_start.size(); ++i) {
...
{
  return isDraw() || isBlackWin() || isWhiteWin();
}
...
bool Board::wouldBeCapture(Loc loc, Player pla) const {
  ...
  if(
    colors[loc + adj_offsets[0]] == opp && colors[loc + adj_offsets[1]] == opp && colors[loc + adj_offsets[4]] == pla) {
    return true;
  }
  if(
    colors[loc + adj_offsets[0]] == opp && colors[loc + adj_offsets[2]] == opp && colors[loc + adj_offsets[5]] == pla) {
    return true;
  }
  if(
    colors[loc + adj_offsets[3]] == opp && colors[loc + adj_offsets[1]] == opp && colors[loc + adj_offsets[6]] == pla) {
    return true;
  }
  if(
    colors[loc + adj_offsets[3]] == opp && colors[loc + adj_offsets[2]] == opp && colors[loc + adj_offsets[7]] == pla) {
    return true;
  }
  return false;
    /*
...
Board::MoveRecord Board::playMoveRecorded(Loc loc, Player pla)
{
  ...
  if(loc != PASS_LOC) {
    ...
    if(
      colors[loc + adj_offsets[0]] == opp && colors[loc + adj_offsets[1]] == opp &&
      colors[loc + adj_offsets[4]] == pla) {
      caps[0] = true;
      caps[1] = true;
    }
    if(
      colors[loc + adj_offsets[0]] == opp && colors[loc + adj_offsets[2]] == opp &&
      colors[loc + adj_offsets[5]] == pla) {
      caps[0] = true;
      caps[2] = true;
    }
    if(
      colors[loc + adj_offsets[3]] == opp && colors[loc + adj_offsets[1]] == opp &&
      colors[loc + adj_offsets[6]] == pla) {
      caps[3] = true;
      caps[1] = true;
    }
    if(
      colors[loc + adj_offsets[3]] == opp && colors[loc + adj_offsets[2]] == opp &&
      colors[loc + adj_offsets[7]] == pla) {
      caps[3] = true;
      caps[2] = true;
    }
...
void Board::undo(Board::MoveRecord record)
{
  ...
  if(chain_data[chain_head[loc]].num_locs > 1) {
    int numNeighbors = 0;
    FOREACHADJ(int adj = loc + ADJOFFSET; if(colors[adj] == record.pla) numNeighbors++;);
    ...
    if(numNeighbors <= 1) {
      ...
      // Extract this move out of the circlar list of stones. Unfortunately we don't have a prev pointer, so we need to
      // walk the loop.
      {
        // Starting at the head is likely to need to walk less since whenever we merge a single stone into an existing
        // group we put it right after the old head.
        Loc cur = head;
      ...
      int libertyDelta = 0;
      FOREACHADJ(int adj = loc + ADJOFFSET; if(colors[adj] == C_EMPTY && !isLibertyOf(adj, head)) libertyDelta--;);
    ...
    // More than one neighbor. Removing this stone potentially disconnects the group into two, so we just do a complete
    // rebuild of the resulting group(s).
    else {
...
Hash128 Board::getPosHashAfterMove(Loc loc, Player pla) const {
  ...
  if(caps[0])
    hash ^= ZOBRIST_BOARD_HASH[loc + adj_offsets[0]][opp];
  if(caps[1])
    hash ^= ZOBRIST_BOARD_HASH[loc + adj_offsets[1]][opp];
  if(caps[2])
    hash ^= ZOBRIST_BOARD_HASH[loc + adj_offsets[2]][opp];
  if(caps[3])
    hash ^= ZOBRIST_BOARD_HASH[loc + adj_offsets[3]][opp];
  return hash;
